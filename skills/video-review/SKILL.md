...
对相邻 clip 的衔接帧（Clip N 尾帧 vs Clip N+1 首帧）以及音轨进行检查，常见 6 类问题：
...
| **VOICE-FIX** | VOICE 类穿帮 | 用第 1 个 clip 提取音色样本 + `clip-regen --voice-asset <id>` 重生该 clip |
...
2. **妆造锁定**：把眉形 / 眼影 / 口红 / 盘发 / 鬓角等细节单独成块，避免被其他描述淹没
3. **道具锁定**：明确列出每个角色手持/佩戴的道具及尺寸形状
4. **场景锁定**：用固定短语描述场景，同一场景的 clip 使用完全相同的描述
5. **范围限制**：只描述当前 15 秒内的事，加"本段仅展示上述动作"
6. **否定约束**：对容易出错的细节加否定描述
7. **主角在场**：每个时段的分镜描述中都要提到主角的动作/位置
8. **音色锚定**：凡有台词必须显式声明 `reference_audio: <voice_asset_id>`，并在生成命令侧 `--voice-asset` 传入
9. **视频参考说明**：传 `--ref-video` 时必须用一句自然语言说明每个参考视频的语义角色（"视频1 是上一段尾部，仅作外观与位置参考，不要复刻其运镜"）
